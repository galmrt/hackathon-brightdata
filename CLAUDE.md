...
**Phase 3 (code side) done**: `services/probe-runner/src/decision.ts` implements the region-agreement + deploy-recency scoring function from §1/§3 — cross-region agreement alone gives 0.6 confidence, a correlated deploy (via a new `deploy-state.json` at repo root, written by `apps/demo-app/scripts/record-deploy.mjs` after every `npm run deploy`) pushes it to 0.95, and only ≥0.9 clears the `auto-heal-candidate` threshold; any partial region disagreement hard-escalates regardless of deploy recency. `src/index.ts` now logs the decision + reasons and includes it in the Port payload (`ProbeRunSummary & { decision: DecisionResult }`); exit code now reflects `decision !== "healthy"` rather than raw region agreement. `/port/blueprints/*.json` draft the three Port blueprints (Probe, ProbeRun, Incident) and `/port/webhook-mapping.json` sketches the ingestion JQ mapping — see `/port/README.md` for the manual import steps and what's unverified. **Still blocked on the Port workspace itself**: no `PORT_WEBHOOK_URL`/API token exists yet, so none of `/port`'s content has been applied or tested against a live workspace — that, plus the actual Port Action for human approval, is Phase 3/4's remaining work.
