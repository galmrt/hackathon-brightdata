...
export function proxyAgentForRegion(region: Region): ProxyAgent {
  const { customerId, zone, apiToken, proxyHost, proxyPort } = config.brightData;
  const username = `brd-customer-${customerId}-zone-${zone}-country-${region.countryCode}`;
  const proxyUrl = `http://${encodeURIComponent(username)}:${encodeURIComponent(apiToken)}@${proxyHost}:${proxyPort}`;
